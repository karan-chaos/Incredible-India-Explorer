/**
 * Anis Kidwai Explorer — Frontend Logic & DOM Controller
 * Handles interactive subnav tabs, memoir excerpt selector, humanitarian milestone filters,
...
/* --- Theme Management --- */
function initTheme() {
    const savedTheme = localStorage.getItem('anis_theme') || 'dark';
    if (savedTheme === 'light') {
...
function toggleTheme() {
    ...
    localStorage.setItem('anis_theme', isLight ? 'light' : 'dark');
    const themeBtn = document.getElementById('theme-toggle');
...
function renderBiography() {
    ...
    if (!container || typeof ANIS_KIDWAI_DATA === 'undefined') return;

    container.innerHTML = ANIS_KIDWAI_DATA.biographySections.map(sec => `
...
function initThesisSimulator() {
    ...
    if (!selectorContainer || !displayContainer || typeof ANIS_KIDWAI_DATA === 'undefined') return;

    const researchList = ANIS_KIDWAI_DATA.thesisResearch;
...
function renderThesisDetail(researchObj) {
    ...
    displayContainer.innerHTML = `
        ...
        <div class="thesis-badge-row">
            <span class="badge-tag">✍️ Memoir Excerpt Analysis</span>
        </div>
...
/* --- Humanitarian Milestones Catalog Filtering --- */
...
function renderMedicalMilestones(category) {
    ...
    const data = ANIS_KIDWAI_DATA.medicalMilestonesCatalog;
...
function renderTimeline() {
    ...
    container.innerHTML = ANIS_KIDWAI_DATA.timelineEvents.map(event => `
...
function renderQuizQuestion() {
    ...
    const questions = ANIS_KIDWAI_DATA.quizQuestions;
...
function evaluateAnswer(selectedIndex, question, optionBtns) {
    ...
    nextBtn.textContent = currentQuestionIndex === ANIS_KIDWAI_DATA.quizQuestions.length - 1 ? 'See Results' : 'Next Question';
