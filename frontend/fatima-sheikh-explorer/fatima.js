/**
 * Fatima Sheikh Explorer — Frontend Logic & DOM Controller
...
/* --- Theme Management --- */
function initTheme() {
    const savedTheme = localStorage.getItem('fatima_theme') || 'dark';
    if (savedTheme === 'light') {
...
function toggleTheme() {
    ...
    localStorage.setItem('fatima_theme', isLight ? 'light' : 'dark');
    const themeBtn = document.getElementById('theme-toggle');
...
function renderBiography() {
    ...
    if (!container || typeof FATIMA_DATA === 'undefined') return;

    container.innerHTML = FATIMA_DATA.biographySections.map(sec => `
...
function initThesisSimulator() {
    ...
    if (!selectorContainer || !displayContainer || typeof FATIMA_DATA === 'undefined') return;

    const researchList = FATIMA_DATA.thesisResearch;
...
function renderThesisDetail(researchObj) {
    ...
    displayContainer.innerHTML = `
        ...
        <div class="thesis-badge-row">
            <span class="badge-tag">✍️ Archival Excerpt &amp; Evidence</span>
        </div>
...
function renderMedicalMilestones(category) {
    ...
    const data = FATIMA_DATA.medicalMilestonesCatalog;
...
function renderTimeline() {
    ...
    container.innerHTML = FATIMA_DATA.timelineEvents.map(event => `
...
function renderQuizQuestion() {
    ...
    const questions = FATIMA_DATA.quizQuestions;
...
function evaluateAnswer(selectedIndex, question, optionBtns) {
    ...
    nextBtn.textContent = currentQuestionIndex === FATIMA_DATA.quizQuestions.length - 1 ? 'See Results' : 'Next Question';
