/* ==========================================================================
   Kamaladevi Chattopadhyay Explorer Logic
   ========================================================================== */
(function () {
    'use strict';
    ...
    function init() {
        ...
        renderFeatureGrid('handicrafts-features', handicraftsFeatures);
        renderFeatureGrid('handicrafts-programs', handicraftsPrograms);
        renderFeatureGrid('handloom-features', handloomFeatures);
        renderFeatureGrid('handloom-traditions', handloomTraditions);
    ...
    function setupBookmark() {
        ...
        btn.addEventListener('click', () => {
            if (window.Journey) {
                window.Journey.toggle({ id, explorerPage: 'frontend/kamaladevi-chattopadhyay/index.html', title: 'Kamaladevi Chattopadhyay', thumbnail: 'https://placehold.co/100/C1440E/fff', category: 'leaders' });
                updateBtnText();
            }
        });
    }
    ...
    function setupScrollAnimations() {
        ...
        if (window.kamaladeviObserver) window.kamaladeviObserver.disconnect();
        window.kamaladeviObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => { if (entry.isIntersecting) { entry.target.classList.add('visible'); window.kamaladeviObserver.unobserve(entry.target); } });
        }, { threshold: 0.1 });
        document.querySelectorAll('.animate-on-scroll').forEach(el => window.kamaladeviObserver.observe(el));
    }

    function setupJourneyIntegration() {
        if (window.Journey && typeof window.Journey.registerSearchItems === 'function') {
            window.Journey.registerSearchItems('frontend/kamaladevi-chattopadhyay/index.html', [{ id: 'leader-kamaladevi', title: 'Kamaladevi Chattopadhyay', description: 'Freedom fighter and reviver of Indian handicrafts.', link: '#' }]);
        }
    }
