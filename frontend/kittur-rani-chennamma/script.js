/* ==========================================================================
   Kittur Rani Chennamma Explorer Logic
   ========================================================================== */
(function () {
    'use strict';

    function init() {
        renderFeatureGrid('kittur-features', kitturFeatures);
        renderFeatureGrid('kittur-political', kitturPolitical);
        renderFeatureGrid('early-life-features', earlyLifeFeatures);
        renderFeatureGrid('early-influences', earlyInfluences);
        renderFeatureGrid('succession-features', successionFeatures);
        renderFeatureGrid('succession-events', successionEvents);
        renderFeatureGrid('eic-features', eicFeatures);
        renderFeatureGrid('eic-demands', eicDemands);
        renderFeatureGrid('resistance-features', resistanceFeatures);
        renderFeatureGrid('resistance-battles', resistanceBattles);
        renderFeatureGrid('fort-features', fortFeatures);
        renderFeatureGrid('fort-architecture', fortArchitecture);
        renderFeatureGrid('capture-features', captureFeatures);
        renderFeatureGrid('capture-imprisonment', captureImprisonment);
    ...
    function setupBookmark() {
        ...
        btn.addEventListener('click', () => {
            if (window.Journey) {
                window.Journey.toggle({ id, explorerPage: 'frontend/kittur-rani-chennamma/index.html', title: 'Kittur Rani Chennamma', thumbnail: 'https://placehold.co/100/8B2500/fff', category: 'leaders' });
                updateBtnText();
            }
        });
    }
    ...
    function setupScrollAnimations() {
        ...
        if (window.chennammaObserver) window.chennammaObserver.disconnect();
        window.chennammaObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => { if (entry.isIntersecting) { entry.target.classList.add('visible'); window.chennammaObserver.unobserve(entry.target); } });
        }, { threshold: 0.1 });
        document.querySelectorAll('.animate-on-scroll').forEach(el => window.chennammaObserver.observe(el));
    }

    function setupJourneyIntegration() {
        if (window.Journey && typeof window.Journey.registerSearchItems === 'function') {
            window.Journey.registerSearchItems('frontend/kittur-rani-chennamma/index.html', [{ id: 'leader-chennamma', title: 'Kittur Rani Chennamma', description: 'Warrior queen who resisted British East India Company in 1824.', link: '#' }]);
        }
    }
