/**
 * Savitribai Phule Explorer — Frontend Logic & DOM Controller
...
/* --- Theme Management --- */
function initTheme() {
    const savedTheme = localStorage.getItem('savitribai_theme') || 'dark';
    if (savedTheme === 'light') {
...
function toggleTheme() {
    ...
    localStorage.setItem('savitribai_theme', isLight ? 'light' : 'dark');
    const themeBtn = document.getElementById('theme-toggle');
...
function renderBiography() {
    ...
    if (!container || typeof SAVITRIBAI_DATA === 'undefined') return;

    container.innerHTML = SAVITRIBAI_DATA.biographySections.map(sec => `
...
function initThesisSimulator() {
    ...
    if (!selectorContainer || !displayContainer || typeof SAVITRIBAI_DATA === 'undefined') return;

    const researchList = SAVITRIBAI_DATA.thesisResearch;
...
function renderThesisDetail(researchObj) {
    ...
    displayContainer.innerHTML = `
        ...
        <div class="thesis-badge-row">
            <span class="badge-tag">✍️ Philosophical Excerpt</span>
        </div>
        ...
        <p style="margin-bottom: 12px;"><strong>Core Message &amp; Concept:</strong> ${researchObj.coreFinding}</p>
...
function renderMedicalMilestones(category) {
    ...
    const data = SAVITRIBAI_DATA.medicalMilestonesCatalog;
...
function renderTimeline() {
    ...
    container.innerHTML = SAVITRIBAI_DATA.timelineEvents.map(event => `
...
function renderQuizQuestion() {
    ...
    const questions = SAVITRIBAI_DATA.quizQuestions;
...
function evaluateAnswer(selectedIndex, question, optionBtns) {
    ...
    nextBtn.textContent = currentQuestionIndex === SAVITRIBAI_DATA.quizQuestions.length - 1 ? 'See Results' : 'Next Question';
